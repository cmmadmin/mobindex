...
function namePatch(name){
    // this patch is to resolve name differences between source data and geodata
    ...
    // "Congo DRC" to "DR Congo"
    if (name == "Congo DRC"){
        return "DR Congo";
    }
    // "C\u00f4te d\u2019Ivoire" to "Republic of C\u00f4te d'Ivoire"
    if (name == "C\u00f4te d\u2019Ivoire"){
        return "Republic of C\u00f4te d'Ivoire";
    }
...
function GeoLocate(input) {
    // intent: given a name, return the latlng array [00, 00]
    input = namePatch(input);
    output = "-";

    var responder = rawdata.filter(function(e) { return e.name.common==input||e.name.official==input;})
    if (responder[0] != null){
        output = responder[0].latlng;
    }
    return output;
}

function GeoRegion(input) {
    // intent: given a name, return the subregion "whereiam"
    input = namePatch(input);
    output = "-";

    var responder = rawdata.filter(function(e) { return e.name.common==input||e.name.official==input;})
    if (responder[0] != null){
        output = responder[0].subregion;
    }
    return output;

}

function GeoDiff(sender, receiver){
    sender = namePatch(sender);
    receiver = namePatch(receiver);

    var responderS = rawdata.filter(function(e) { return e.name.common==sender||e.name.official==sender;})
    if ((responderS[0] == null) || (responderS[0].latlng == null)|| !("latlng" in responderS[0])){
        outputS = "-";
    }
    else {
        outputS = responderS[0].latlng;
    }

    var responderR = rawdata.filter(function(e) { return e.name.common==receiver||e.name.official==receiver;})
    if ((responderR[0] == null) || (responderR[0].latlng == null)|| !("latlng" in responderR[0])){
        outputR = "-";
    }
    else {
        outputR = responderR[0].latlng;
    }

    if ( (responderR == "-") || (responderS == "-") ) {
        return "-";
    }
    else {
        // do math, return diff
        var radlat1 = Math.PI * outputR[0]/180
        var radlat2 = Math.PI * outputS[0]/180
        var theta = outputR[1]-outputS[1]
        ...
        //return outputS + ", " + outputR + "= " + output;  // previous methodology
    }
}
