    ...
    <script type="text/javascript" class="init">
        ...
        $(document).ready(function() {
            $('#submitted').DataTable({ //This first table is just for showing submitted row
                ...
                columns: [{
                    title: "Country"
                }, /* table col 0, data source = 0 */ {
                    title: "Power Distance"
                }, /* table col 1, data source = 1 */ {
                    title: "Individualism"
                }, /* table col 2, data source = 2 */ {
                    title: "Masculinity"
                }, /* table col 3, data source = 3 */ {
                    title: "Uncertainty Avoidance"
                }, /* table col 4, data source = 4 */ {
                    title: "Long Term Orientation"
                }, /* table col 5, data source = 5 */ {
                    title: "Indulgence"
                }, /* table col 6, data source = 6 */ {
                    title: "Average Median Cultural Distance to UPG's"
                } /* table col 9, data source = calculated, "data" column is placeholder*/ ],
                "columnDefs": [{
        ...
        $(document).ready(function() {
            $('#upgcompare').DataTable({
                ...
                "createdRow": function(row, data, index) {
                    ...
                    if (isEmpty(dataSub[0][1]) || isEmpty(data[1])) {} else if (Math.abs(dataSub[0][1] - data[1]) > 19) {
                        $('td', row).eq(1).addClass('high');
                    } else if (Math.abs(dataSub[0][1] - data[1]) < 11) {
                        $('td', row).eq(1).addClass('low');
                    }
                    if (isEmpty(dataSub[0][2]) || isEmpty(data[2])) {} else if (Math.abs(dataSub[0][2] - data[2]) > 19) {
                        $('td', row).eq(2).addClass('high');
                    } else if (Math.abs(dataSub[0][2] - data[2]) < 11) {
                        $('td', row).eq(2).addClass('low');
                    }
                    if (isEmpty(dataSub[0][3]) || isEmpty(data[3])) {} else if (Math.abs(dataSub[0][3] - data[3]) > 19) {
                        $('td', row).eq(3).addClass('high');
                    } else if (Math.abs(dataSub[0][3] - data[3]) < 11) {
                        $('td', row).eq(3).addClass('low');
                    }
                    if (isEmpty(dataSub[0][4]) || isEmpty(data[4])) {} else if (Math.abs(dataSub[0][4] - data[4]) > 19) {
                        $('td', row).eq(4).addClass('high');
                    } else if (Math.abs(dataSub[0][4] - data[4]) < 11) {
                        $('td', row).eq(4).addClass('low');
                    }
                    if (isEmpty(dataSub[0][5]) || isEmpty(data[5])) {} else if (Math.abs(dataSub[0][5] - data[5]) > 19) {
                        $('td', row).eq(5).addClass('high');
                    } else if (Math.abs(dataSub[0][5] - data[5]) < 11) {
                        $('td', row).eq(5).addClass('low');
                    }
                    if (isEmpty(dataSub[0][6]) || isEmpty(data[6])) {} else if (Math.abs(dataSub[0][6] - data[6]) > 19) {
                        $('td', row).eq(6).addClass('high');
                    } else if (Math.abs(dataSub[0][6] - data[6]) < 11) {
                        $('td', row).eq(6).addClass('low');
                    }
                    if (isEmpty(dataSub[0][7]) || isEmpty(data[7])) {} else if (row[7] > 19) {
                        $('td', row).eq(7).addClass('high');
                    } // This is not working properly...
                    else if (row[7] < 11) {
                        $('td', row).eq(7).addClass('low');
                    } // This is not working properly...
